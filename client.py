...
from loss import unsupervised_loss, src_loss
    ...
    def __train_unsupervised(self, epochs: int):
        ...
        for _ in range(epochs):
            for noised_X, X, __ in self.dataloader['unlabeled']:
                ...
                noised_pred,local_last_feature_map = model(confident_noised_X)
                helper_preds,helper_last_feature_maps = self.__helper_predictions(confident_X)

                loss = loss_fn(self.params["sigma"], self.params["phi"], confident_pred,
                               noised_pred, helper_preds, 
                               local_last_feature_map, helper_last_feature_maps,
    ...
    def __helper_predictions(self, X: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            for helper_phi in self.helpers_phi:
                ...
                pred,helper_last_feature_map = model(X)
